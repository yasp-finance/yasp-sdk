...
export class VaultAccounts implements IVaultAccounts {
  constructor(public strategyVaultProgram: Program<VaultProgram>) {}
  ...
  vaultStrategy(strategyMintAddress: PublicKey): Promise<VaultStrategy> {
    const fetch: VaultStrategyFetch =
      this.strategyVaultProgram.account.vaultStrategy.fetch
    return fetch(strategyMintAddress)
  }

  vaultFeeDistributor(
    feeDistributorAddress: PublicKey
  ): Promise<VaultFeeDistributor> {
    const fetch: VaultFeeDistributorFetch =
      this.strategyVaultProgram.account.vaultFeeDistributor.fetch
